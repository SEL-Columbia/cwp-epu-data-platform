...
| **options.isDefault** [`boolean`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Boolean) | If `true`, this vector layer will be selected when the page loads |
...
| **options.customLegendsByName** [`Object`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object) | An object that maps raster layer names (from `options.nameVariable`) to an array of objects, each with the `name` and `color` of a block in the legend. |
